...
```{r}
getwd()
mgCSTs.samples.meta<-as.data.frame(read_excel("RDS_files/Supplemental_File_6.xlsx", skip = 3, na = c("NA", "#N/A")))
...
names(samples.clusters)[5]<-"Ca._Lachnocurva_vaginae"
...
for (i in c("Lactobacillus_crispatus", "Lactobacillus_gasseri", "Lactobacillus_jensenii", "Lactobacillus_iners", "Ca._Lachnocurva_vaginae")){
  ...
  saveRDS(tuning, paste("RDS_files/", i, "_fineTuning.RDS", sep=""))
}
...
for (n in unique(li.s$mgss)){
  ...
  tab <- cbind(mgCST=n, X2 = as.data.frame(m["statistic"]), p = as.data.frame(m["p.value"]), nSamples=nrow(test), propBV = as.vector(prop.table(table(test$bv)))[1], prop_noBV = as.vector(prop.table(table(test$bv)))[2],  test="X2")
  rownames(tab)<-tab["mgCST"]
  if (exists("bvornot")){
